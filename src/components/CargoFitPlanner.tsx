...
export default function CargoFitPlanner() {
  ...
  if (roundedTotalVolume > 0) {
    if (roundedTotalVolume <= 5.0) {
      ...
      statusText = "ฟิตพอดี (จัดวางแน่นหนา)";
  ...
  return (
    <div className="rounded-2xl border border-slate-200/90 p-6 sm:p-8 bg-white shadow-xs">
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 space-y-6">
          <div className="flex justify-between items-center border-b border-slate-100 pb-4">
            <div>
              <span className="text-xs text-blue-700 tracking-wide font-bold uppercase">WMS CARGO FIT SCORE</span>
              <h3 className="text-xl font-extrabold text-[#0B1F3A]">เครื่องคำนวณปริมาตร คิว (CBM)</h3>
            </div>
            {roundedTotalVolume > 0 && (
              <button 
                onClick={clearInventory}
                className="flex items-center gap-1.5 text-xs text-slate-600 hover:text-slate-900 transition-colors border border-slate-200 px-3 py-1.5 rounded-lg bg-slate-50 cursor-pointer"
              >
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-end">
              <span className="text-sm font-semibold text-slate-700">ปริมาตรรวมที่ใช้:</span>
              <div className="text-right">
                <span className="text-2xl font-black text-[#0B1F3A]">{roundedTotalVolume} </span>
                <span className="text-xs font-bold text-slate-500">/ {maxVolume} CBM (คิว)</span>
              </div>
            </div>
            
            {/* Visual Progress Bar */}
            <div className="w-full h-4 bg-slate-100 border border-slate-200 rounded-full overflow-hidden relative">
              <div 
                className="h-full bg-linear-to-r from-blue-600 to-blue-500 transition-all duration-300 rounded-full"
                style={{ width: `${fillPercentage}%` }}
              />
              <span className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-slate-700 tracking-wide">
                {fillPercentage}%
              </span>
            </div>
          </div>
          ...
          <div className={`border rounded-xl p-4 sm:p-5 ${statusBg} transition-all duration-200`}>
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className={`text-base font-bold ${statusColor}`}>{statusText}</h4>
                <p className="text-xs sm:text-sm text-slate-700 leading-relaxed mt-1 font-medium">{advice}</p>
              </div>
            </div>
          </div>
          ...
          <div className="border border-slate-200 bg-slate-50 rounded-xl p-4 space-y-3">
            <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">สเปกขนาดรถอ้างอิง (ผ่านการรับรองแล้ว)</h4>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
              <div className="border-r border-slate-200 last:border-0 pr-2">
                ...
                <span className="text-sm font-extrabold text-[#0B1F3A]">{truckSpec.dimensions.widthInternalMeters} ม.</span>
              </div>
              <div className="border-r border-slate-200 last:border-0 pr-2">
                <span className="text-[10px] text-slate-500 font-bold block">ยาวภายใน</span>
                <span className="text-sm font-extrabold text-[#0B1F3A]">{truckSpec.dimensions.lengthInternalMeters} ม.</span>
              </div>
              <div className="border-r border-slate-200 last:border-0 pr-2">
                <span className="text-[10px] text-slate-500 font-bold block">สูงภายใน</span>
                <span className="text-sm font-extrabold text-[#0B1F3A]">{truckSpec.dimensions.heightInternalMeters} ม.</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-500 font-bold block">รับน้ำหนักสูงสุด</span>
                <span className="text-sm font-extrabold text-[#0B1F3A]">{truckSpec.dimensions.payloadCapacityKg} กก.</span>
              </div>
            </div>
            <div className="text-[10px] text-slate-500 font-medium italic text-center pt-1">
        ...
        <div className="flex-1 space-y-4">
          <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">เลือกรายการสิ่งของของคุณ</h4>
          
          <div className="h-[280px] sm:h-[350px] overflow-y-auto pr-2 space-y-2 border border-slate-200 rounded-xl p-3 bg-slate-50/50">
            {INVENTORY_ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  <div>
                    <span className="text-xs sm:text-sm text-slate-900 font-bold block">{item.name}</span>
                    <span className="text-[10px] text-slate-500 font-medium block">ปริมาตร: {item.volumeCbm} คิว (CBM)</span>
                  </div>
                  <div className="flex items-center gap-2">
                    {qty > 0 && (
                      <button 
                        onClick={() => updateQty(item.id, -1)}
                        className="p-1 hover:bg-slate-100 border border-slate-200 rounded-md text-slate-600 cursor-pointer"
                      >
                        <Minus className="h-3 w-3" />
                      </button>
                    )}
                    {qty > 0 && (
                      <span className="w-6 text-center font-mono font-bold text-sm text-blue-700">{qty}</span>
                    )}
                    <button 
                      onClick={() => updateQty(item.id, 1)}
                      className="p-1 bg-blue-600 hover:bg-blue-700 border border-blue-600 rounded-md text-white shadow-xs cursor-pointer"
                    >
          ...
          {roundedTotalVolume > 0 && (
            <div className="bg-slate-100 border border-slate-200 rounded-xl p-3.5 flex justify-between items-center text-xs">
              <span className="text-slate-700 font-bold">สรุปสิ่งของทั้งหมด: {Object.values(selectedItems).reduce((a, b) => a + b, 0)} ชิ้น</span>
              <a 
                ...
                className="text-xs bg-[#06C755] hover:bg-[#05b34c] text-white font-bold px-3.5 py-1.5 rounded-lg transition-colors inline-flex items-center gap-1.5 shadow-xs"
              >
